...
if ! grep -q "^B: REL=" /proc/bus/input/devices; then
  echo "relative pointer capability is missing from /proc/bus/input/devices" >&2
  exit 1
fi

if ! { test -S /tmp/game-runtime/wayland-0 || test -S /tmp/weston-runtime/wayland-0 || test -S /run/user/0/wayland-0 || test -S /run/wayland-0; }; then
  echo "gamescope/Weston did not expose a Wayland socket" >&2
  exit 1
fi

if ! { pgrep -x gamescope >/dev/null 2>&1 || pgrep -x weston >/dev/null 2>&1; }; then
  echo "neither gamescope nor Weston is running" >&2
  exit 1
fi
...
if ! command -v vulkaninfo >/dev/null 2>&1; then
  echo "vulkaninfo is not installed" >&2
  exit 1
fi

vulkaninfo --summary >/tmp/smolvm-game-vulkaninfo.summary 2>&1 || {
  echo "vulkaninfo failed; see /tmp/smolvm-game-vulkaninfo.summary" >&2
  exit 1
}
'
