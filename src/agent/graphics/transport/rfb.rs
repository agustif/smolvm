//! RFB transport for the native libkrun display/input bridge.
//!
...
pub fn endpoint_socket(machine_name: &str) -> PathBuf {
    crate::agent::vm_data_dir(machine_name)
...
pub fn read_endpoint(machine_name: &str) -> Result<DisplayEndpoint, String> {
    decode_endpoint(&read_endpoint_json(machine_name)?)
}

/// Reads and minimally validates the current endpoint JSON for any display transport.
pub fn read_endpoint_json(machine_name: &str) -> Result<Vec<u8>, String> {
    ...
    validate_endpoint_json(&bytes)?;
    Ok(bytes)
}

fn validate_endpoint_json(bytes: &[u8]) -> Result<(), String> {
    if bytes.len() > 4_096 {
    ...
    let protocol = value
        ...
        .ok_or_else(|| "display endpoint is missing protocol".to_string())?;
    match protocol {
        "vnc" | "local-shm" => Ok(()),
        _ => Err("display endpoint failed validation".into()),
    }
}
...
mod tests {
    ...
    fn endpoint_decoder_accepts_only_bounded_loopback_vnc() {
        ...
        validate_endpoint_json(valid).unwrap();
        validate_endpoint_json(
            br#"{"protocol":"local-shm","version":1,"control_socket":"/tmp/display/local-shm.sock","frame_ring":"/tmp/display/local-shm.frames"}"#,
        )
        .unwrap();
        ...
        assert!(decode_endpoint(&oversized)
            .unwrap_err()
            .contains("4096-byte"));
        assert!(validate_endpoint_json(&oversized)
            .unwrap_err()
            .contains("4096-byte"));
    }
