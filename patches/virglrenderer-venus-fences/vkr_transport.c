...
static bool
vkr_cs_remainder_is_padding(const struct vkr_cs_decoder *dec)
{
   const uint8_t *p = dec->cur;
...
static void
vkr_log_cs_hex(const char *tag, const uint8_t *p, size_t n)
{
   char buf[3 * 64 + 8];
   size_t m = n < 64 ? n : 64;
   char *o = buf;
   *o = '\0';
   for (size_t i = 0; i < m; i++)
      o += sprintf(o, "%02x ", p[i]);
   vkr_log("%s bytes=%zu hex=%s", tag, n, buf);
}
   ...
   struct vn_command_vkExecuteCommandStreamsMESA *args)
{
   ...
   for (uint32_t i = 0; i < args->streamCount; i++) {
      ...
      /* Guest reuses CS shmem. Decode a snapshot so a wrap/reuse cannot
       * turn leftover zeros into vkCreateInstance (cmd_type 0).
       */
      uint8_t *copy = malloc(stream->size);
      if (!copy) {
         vkr_log("failed to copy command stream %u res_id %u size %u", i,
                 stream->resourceId, stream->size);
         vkr_context_set_fatal(ctx);
         break;
      }
      memcpy(copy, res->u.data + stream->offset, stream->size);
      vkr_cs_decoder_set_stream(dec, copy, stream->size);
      while (vkr_cs_decoder_has_command(dec)) {
         if (vkr_cs_remainder_is_padding(dec))
            break;
         vn_dispatch_command(dispatch);
         if (vkr_context_get_fatal(ctx)) {
            vkr_log("CS fatal stream %u res_id %u off=%u size=%u remain=%zu", i,
                    stream->resourceId, stream->offset, stream->size,
                    (size_t)(dec->end - dec->cur));
            vkr_log_cs_hex("CS fatal at", dec->cur, (size_t)(dec->end - dec->cur));
            vkr_log_cs_hex("CS fatal start", copy, stream->size);
            break;
         }
      }
      free(copy);
