...
{
   /* In-process vkr requires VKR_RENDERER_ASYNC_FENCE_CB. Signaled fences
    * are retired from venus_state_cb_retire_fence on the vkr queue thread.
    * virgl_renderer_poll still calls this when host ASYNC_FENCE_CB is off;
    * there is no seqno shmem to poll, so this is a no-op (same as msm).
    */
   (void)base;
}
...
venus_state_cb_retire_fence(uint32_t ctx_id, uint32_t ring_idx, uint64_t fence_id)
{
   ...
   /* vkr already waited. Retire this virtio fence id (and any older
    * mergeable fences on the same timeline). Do not treat fence_id as
    * timeline seqno — they are different counters.
    */
   if (ring_idx >= VENUS_CONTEXT_TIMELINE_COUNT) {
   ...
   struct venus_timeline *timeline = &ctx->timelines[ring_idx];
   bool retired_target = false;
   list_for_each_entry_safe (struct venus_fence, fence, &timeline->fences, head) {
      const uint64_t id = fence->fence_id;
      list_del(&fence->head);
      venus_context_free_fence(ctx, fence);
      ctx->base.fence_retire(&ctx->base, ring_idx, id);
      if (id == fence_id) {
         retired_target = true;
         break;
      }
   ...
   if (list_is_empty(&timeline->fences))
      ctx->timeline_busy_mask &= ~(1ull << ring_idx);
