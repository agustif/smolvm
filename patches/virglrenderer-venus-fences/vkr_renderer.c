...
vkr_get_capset(void *capset)
{
   ...
   if (c) {
      ...
      /* Blocking vkWaitForFences on the Venus ring thread deadlocks when the
       * waited work still needs more ring commands (typical WSI acquire).
       * Guest Mesa then polls vkGetFenceStatus instead.
       */
      c->allow_vk_wait_syncs = 0;
