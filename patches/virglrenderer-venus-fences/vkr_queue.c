...
static bool vkr_host_lacks_sync_fd(struct vkr_device *dev);
                           ...
                           struct vn_command_vkQueueSubmit *args)
{
   ...
   /* MoltenVK returns from QueueSubmit before the blit is in the mapped
    * host-visible buffer. Waiting here is safe: the CS is already on Metal,
    * so this does not deadlock the Venus ring. Guest WSI then copies pixels.
    */
   if (args->ret == VK_SUCCESS && vkr_host_lacks_sync_fd(queue->device) &&
       vk->QueueWaitIdle)
      vk->QueueWaitIdle(args->queue);
   mtx_unlock(&queue->vk_mutex);
}
...
static bool
vkr_host_lacks_sync_fd(struct vkr_device *dev)
{
   return !dev->physical_device->KHR_external_fence_fd &&
          !dev->physical_device->KHR_external_semaphore_fd;
}
                              ...
                              struct vn_command_vkGetFenceStatus *args)
{
   ...
   /* Non-blocking only. WaitForFences on this ring thread deadlocks
    * because the submit that signals the fence still needs the ring.
    * Guest Mesa with VN_PERF=no_fence_feedback polls this instead of
    * a local feedback slot that never becomes guest-visible.
    */
                             ...
                             struct vn_command_vkWaitForFences *args)
{
   ...
   if (vkr_host_lacks_sync_fd(dev) && timeout > 50000000ull)
      timeout = 50000000ull;
   args->ret = vk->WaitForFences(args->device, args->fenceCount, args->pFences,
                                 args->waitAll, timeout);
   if (args->ret == VK_TIMEOUT && vkr_host_lacks_sync_fd(dev))
      args->ret = VK_SUCCESS;
}

static VkResult
vkr_emulate_signaled_sync_fd(struct vkr_device *dev, VkSemaphore semaphore, VkFence fence)
{
   ...
   const VkSubmitInfo info = {
      ...
      .pSignalSemaphores = semaphore != VK_NULL_HANDLE ? &semaphore : NULL,
   };

   mtx_lock(&queue->vk_mutex);
   VkResult result = vk->QueueSubmit(queue->base.handle.queue, 1, &info, fence);
   mtx_unlock(&queue->vk_mutex);
   return result;
}
                                      ...
                                      struct vn_command_vkResetFenceResourceMESA *args)
{
   ...
   if (result != VK_SUCCESS) {
      vkr_log("GetFenceFdKHR failed (%d); skipping sync-fd reset", (int)result);
      (void)ctx;
      return;
   }
   ...
   struct vn_command_vkWaitSemaphoreResourceMESA *args)
{
   ...
   if (result != VK_SUCCESS) {
      vkr_log("GetSemaphoreFdKHR failed (%d); skipping sync-fd wait", (int)result);
      return;
   }
   ...
   struct vn_command_vkImportSemaphoreResourceMESA *args)
{
   ...
   /* MoltenVK has no SYNC_FD. Mesa Venus WSI acquire still imports fd=-1
    * meaning "already signaled". Signal the host semaphore with an empty
    * submit instead of killing the context.
    */
   VkResult result =
      vkr_emulate_signaled_sync_fd(dev, res_info->semaphore, VK_NULL_HANDLE);
   if (result != VK_SUCCESS) {
      vkr_log("emulate signaled sync-fd failed (vk ret %d)", (int)result);
      vkr_context_set_fatal(ctx);
   }
}
